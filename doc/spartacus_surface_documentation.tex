...
transfer calculation.  This results in a calculation of the
...
takes as input three Fortran derived types describing (1) the
wavelength-independent properties of the canopy, (2) the shortwave
spectral properties and (3) the longwave spectral properties.  Each
object describes \code{ncol} surface `columns' to be treated
independently; each column could correspond to an atmospheric model
column, or alternatively may represent multiply `tiles' lying under
one or more atmospheric columns. Since it is envisaged that many of
these columns
...
\begin{lstlisting}
 ...
 ! Canopy flux components, the last three normalized by top-of-canopy downwelling
...
\tablesetup
\begin{longtable}{lll}
\caption{\label{tab:canopy_props}}
\hline
Variable & dimensions
\codetab{nlay}
\codetab{istartlay}
\codetab{dz}
\codetab{cos\_sza}
\codetab{ground\_temperature}
\codetab{roof\_temperature
    \end{longtable}
\end{center}
\fi
