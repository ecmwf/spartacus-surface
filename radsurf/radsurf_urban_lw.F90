      ...
      do jlay = 1,nlay
        ...
        do jreg = 1,nreg
          if (frac(jreg,jlay) <= config%min_vegetation_fraction) then
            ...
            f_wall(jreg,jlay) = norm_perim_wall(jreg,jlay) * lg%vadjustment2 &
                 &            / (Pi * frac(jreg,jlay))
          end if
        end do
        ...
        do jreg = 1,nreg
          volume_emiss = frac(jreg,jlay) * (ext_reg(:,jreg) * (1.0_jprb - ssa_reg(:,jreg)) &
               &                            * planck_reg(:,jreg))
          wall_emiss = norm_perim_wall(jreg,jlay) * lg%vadjustment &
               &                             * wall_emission(:,jlay)
          do js = 1,ns
            ifr = js + (jreg-1)*ns
        ...
        emiss_wall(:,jlay) = (sum(norm_perim_wall(:,jlay)) * lg%vadjustment) * wall_emission(:,jlay)
